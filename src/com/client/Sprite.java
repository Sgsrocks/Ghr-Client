...
public class Sprite extends Rasterizer2D {
	...
	public Sprite(byte abyte0[], Component component) {
		try {
			image = Toolkit.getDefaultToolkit().createImage(this.location + "Background.png");
...
//	}
	...
	public void drawSpriteWithDimens(int xPos, int yPos, int vertSize) {
		xPos += anInt1442;
		yPos += anInt1443;
		int l = xPos + yPos * Rasterizer2D.width;
		int i1 = 0;
		int horizSize = myWidth;
		int l1 = Rasterizer2D.width - horizSize;
		int i2 = 0;
		if (yPos < Rasterizer2D.clip_top) {
			int j2 = Rasterizer2D.clip_top - yPos;
			vertSize -= j2;
			yPos = Rasterizer2D.clip_top;
			i1 += j2 * horizSize;
			l += j2 * Rasterizer2D.width;
		}
		if (yPos + vertSize > Rasterizer2D.clip_bottom)
			vertSize -= (yPos + vertSize) - Rasterizer2D.clip_bottom;
		if (xPos < Rasterizer2D.clip_left) {
			int k2 = Rasterizer2D.clip_left - xPos;
			horizSize -= k2;
		...
		if (xPos + horizSize > Rasterizer2D.clip_right) {
			int l2 = (xPos + horizSize) - Rasterizer2D.clip_right;
			horizSize -= l2;
			i2 += l2;
			l1 += l2;
		}
		if (!(horizSize <= 0 || vertSize <= 0)) {
			method349(Rasterizer2D.pixels, myPixels, i1, l, horizSize, vertSize,
					l1, i2);
		}
	}
	...
	public void drawARGBSprite2(int xPos, int yPos, int alpha) {
		...
		if (!(spriteWidth <= 0 || spriteHeight <= 0)) {
			renderARGBPixels2(spriteWidth, spriteHeight, myPixels, Rasterizer2D.pixels, i1, alphaValue, j1, j2, i2);
		}
	}
	...
	public void drawAdvancedTransparentSprite(int i, int j, int opacity) {
		...
		if (!(l1 <= 0 || k1 <= 0)) {
			drawAlphaSprite(j1, l1, Rasterizer2D.pixels, myPixels, j2, k1, i2, k, i1);
		}
	}
	...
	public Sprite(FileArchive streamLoader, String s, int i) {
...
//		}
	...
	public void method343() {
		Rasterizer2D.initDrawingArea(myHeight, myWidth, myPixels, new float[1024]);
	}
	...
	public void method346(int i, int j) {
		...
		int i2 = 0;
		if (j < Rasterizer2D.clip_top) {
			int j2 = Rasterizer2D.clip_top - j;
			j1 -= j2;
			j = Rasterizer2D.clip_top;
			i1 += j2 * k1;
			l += j2 * Rasterizer2D.width;
		}
		if (j + j1 > Rasterizer2D.clip_bottom)
			j1 -= (j + j1) - Rasterizer2D.clip_bottom;
		if (i < Rasterizer2D.clip_left) {
		...
		if (k1 <= 0 || j1 <= 0) {
			...
			method347(l, k1, j1, i2, i1, l1, myPixels, Rasterizer2D.pixels);
		}
	}
	...
	public void drawSprite(int i, int k, int color) {
		...
		if (!(k1 <= 0 || j1 <= 0)) {
			try {
				method349(Rasterizer2D.pixels, raster, i1, l, k1, j1, l1, i2);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public void drawSprite(int i, int k) {
		...
		if (!(k1 <= 0 || j1 <= 0)) {
			method349(Rasterizer2D.pixels, myPixels, i1, l, k1, j1, l1, i2);
		}
	}
		...
		try {
			...
			for (j1 = 0; j1 < i; j1++) {
				...
				for (k1 = -ai[j1]; k1 < 0; k1++) {
					Rasterizer2D.pixels[j4++] = myPixels[(k4 >> 16) + (l4 >> 16)
							* myWidth];
					k4 += i3;
					l4 -= l2;
				}
	...
	public void method354(IndexedImage background, int i, int j) {
		...
		int l1 = 0;
		if (i < Rasterizer2D.clip_top) {
			int i2 = Rasterizer2D.clip_top - i;
			i1 -= i2;
			i = Rasterizer2D.clip_top;
			l += i2 * j1;
			k += i2 * Rasterizer2D.width;
		}
		if (i + i1 > Rasterizer2D.clip_bottom)
			i1 -= (i + i1) - Rasterizer2D.clip_bottom;
		if (j < Rasterizer2D.clip_left) {
			int j2 = Rasterizer2D.clip_left - j;
			j1 -= j2;
		...
		if (j + j1 > Rasterizer2D.clip_right) {
			int k2 = (j + j1) - Rasterizer2D.clip_right;
			j1 -= k2;
			l1 += k2;
			k1 += k2;
		}
		if (!(j1 <= 0 || i1 <= 0)) {
			method355(myPixels, j1, background.palettePixels, i1,
					Rasterizer2D.pixels, 0, k1, k, l1, l);
		}
	}
	...
	public void drawSpriteWithOpacity(int xPos, int yPos, int o) {
		...
		if (yPos + k1 > Rasterizer2D.clip_bottom)
			k1 -= (yPos + k1) - Rasterizer2D.clip_bottom;
		if (xPos < Rasterizer2D.clip_left) {
			int l2 = Rasterizer2D.clip_left - xPos;
			l1 -= l2;
			xPos = Rasterizer2D.clip_left;
		...
		if (xPos + l1 > Rasterizer2D.clip_right) {
			int i3 = (xPos + l1) - Rasterizer2D.clip_right;
			l1 -= i3;
			j2 += i3;
			i2 += i3;
		}
		if (!(l1 <= 0 || k1 <= 0)) {
			method351(j1, l1, Rasterizer2D.pixels, myPixels, j2, k1, i2, o, i1);
		}
	}
