...
public class RSInterface {
	...
	/*
	addHoverButton = (int i, String imageName, int j, int width, int height, String text, int contentType, int hoverOver, int aT)
	addHoveredButton = (int i, String imageName, int j, int w, int h, int IMAGEID)
	addText = (int id, String text, TextDrawingArea wid[], int idx, int color)
	 */
	public static void MusicList(TextDrawingArea[] tda) {
		for (int i = 0; i < 3; i++) {
			RSInterface scroll = addInterface(38163 + i);
			scroll.totalChildren(100);
			for (int j = 0; j < 100; j++) {
				addClickableText(56037 + j + (i * 100), "Achievement: " + j, "Select", tda, 0, 0xff9040, false, true,
						112);
				scroll.child(j, 56037 + j + (i * 100), 2, 4 + (j * 13));
			}
			scroll.width = 109;
			scroll.height = 232;
	...
	public static void skillMenu(TextDrawingArea[] tda) {
		RSInterface tab = addInterface(8000);
		
		addSprite(8001, 3, "Interfaces/skillMenu/BACKGROUND"); //Background
		
		addHoverButton(8002, "Interfaces/skillMenu/CLOSE", 1, 16, 16, "Close", -1, 8003, 1); //Exit Button
		addHoveredButton(8003, "Interfaces/skillMenu/CLOSE", 2, 16, 16, 8004); //Exit Button
		
		addSprite(8005, 2, "Interfaces/skillMenu/TEST"); //Skill Sprite
		addSprite(8006, 1, "Interfaces/skillMenu/SKILL"); //Necromancy Sprite
		
		addText(8007, "Level:", tda, 2, 0xff9040, true, true); //Text
		addText(8008, "", tda, 2, 0xff9040, true, true); //Text
		
		addHoverButton(8009, "Interfaces/skillMenu/BUTTON", 3, 35, 25, "Spawn", -1, 8010, 1); //Spawn Button
		addHoveredButton(8010, "Interfaces/skillMenu/BUTTON", 4, 35, 25, 8011); //Spawn Button
		
		addSprite(8012, 1, "Interfaces/skillMenu/SPRITE"); //Spawn Tab Sprite
		
		addSprite(8013, 3, "Interfaces/skillMenu/TEST"); //Skill Sprite
		
		addText(8014, "EXP:", tda, 2, 0xff9040, true, true); //Text
		addText(8015, "", tda, 2, 0xff9040, true, true); //Text
		
		setChildren(13, tab);
		setBounds(8001, 5, 5, 0, tab);
		setBounds(8002, 140, 250, 1, tab);
		setBounds(8003, 140, 250, 2, tab);
		setBounds(8005, 12, 12, 3, tab);
		setBounds(8006, 134, 30, 4, tab);
		setBounds(8007, 40, 15, 5, tab);
		setBounds(8008, 100, 15, 6, tab);
		
		setBounds(8009, 35, 54, 7, tab);
		setBounds(8010, 35, 54, 8, tab);
		setBounds(8012, 40, 54, 9, tab);
		setBounds(8013, 150, 12, 10, tab);
		
		setBounds(8014, 170, 15, 11, tab);
		setBounds(8015, 235, 15, 12, tab);
		
		RSInterface tab1 = addInterface(9000);
		addSprite(9001, 0, "Interfaces/Bankpin/IMAGE");
		addText(9002, "Enter Ammount", tda, 2, 0xFF981F, true, true);
		addText(9003, "Enter The Ammount", tda, 1, 0xFF981F, true, true);
		addText(9004, "Of Npcs You Would", tda, 1, 0xFF981F, true, true);
		addText(9005, "Like To Spawn", tda, 1, 0xFF981F, true, true);
		addText(9006, "Press enter to submit", tda, 2, 0xFF981F, true, true);
		addInputField(9007, 8, 0xFF981F, "", 100, 24, false);
		addHoverButton(9008, "Interfaces/Bankpin/IMAGE", 1, 16, 16, "Close", 375, 9009, 3);
		addHoveredButton(9009, "Interfaces/Bankpin/IMAGE", 2, 16, 16, 9010);
		setChildren(9, tab1);
		setBounds(9001, 256 - 140, 120, 0, tab1);
		setBounds(9002, 256, 132, 1, tab1);
		setBounds(9003, 256, 150, 2, tab1);
		setBounds(9004, 256, 165, 3, tab1);
		setBounds(9005, 256, 180, 4, tab1);
		setBounds(9006, 256, 200, 5, tab1);
		setBounds(9007, 256 - 50, 220, 6, tab1);
		setBounds(9008, 374, 127, 7, tab1);
		setBounds(9009, 374, 127, 8, tab1);
	}
	...
	public static void achievementPopup2(TextDrawingArea[] tda) {
		...
		addSprite(56001, 0, dir);
		addHoverButton(56002, dir, 1, 21, 21, "Close", -1, 56003, 1);
		addHoveredButton(56003, dir, 2, 21, 21, 56004);
		addText(56005, "Achievement Viewer", tda, 2, 0xff9040, true, true);
		addText(56550, "All rewards for completing this achievement:", tda, 0, 0xFFA500, false, true);
		addText(56026, "Easy", tda, 2, 0xff9040, true, true);
		int x = 5, y = 5;
		tab.totalChildren(19);
		tab.child(0, 56001, x, y);
		tab.child(1, 56002, 669 + x, 7 + y);
		tab.child(2, 56003, 669 + x, 7 + y);
		tab.child(3, 56005, 248 + x, 10 + y);
		String[] titles = { "Easy", "Medium", "Hard" };
		int xx = 7;
		for (int i = 0; i < 3; i++) {
			addHoverButton(66006 + i, dir, 3, 160, 20, "View", -1, 56010 + i, 1);
			addHoveredButton(66010 + i, dir, 4, 160, 20, 56014 + i);
			addText(66018 + i, titles[i], tda, 1, 0xff7000, true, true);
			tab.child(4 + i, 66006 + i, xx + x, 56 + y);
			tab.child(7 + i, 66010 + i, xx + x, 56 + y);
			tab.child(10 + i, 66018 + i, xx + x + 80, 40 + y);
			xx += 161;
		}
		tab.child(13, 56023, 8 + x, 82 + y);
		tab.child(14, 56500, 152 + x, 68 + y);
		tab.child(15, 56510, 317 + x, 232 + y);
		tab.child(16, 56520, 152 + x, 232 + y);
		tab.child(17, 56550, 171 + x, 218 + y);
		tab.child(18, 56026, 70 + x, 62 + y);

		RSInterface info = addInterface(56500);
		addText(56501, "Title Of Achievement", tda, 3, 0xff9040, true, true);
		addText(56502, "Progress: @gre@0% (0/0)", tda, 1, 0xffffff, true, true);
		info.totalChildren(8);
		info.child(0, 56501, 163, 6);
		info.child(1, 56502, 163, 26);
		for (int i = 0; i < 6; i++) {
			addText(56503 + i, "Description #" + i, tda, 0, 0xffffff, true, true);
			info.child(2 + i, 56503 + i, 163, 51 + (i * 13));
		}

		RSInterface exp = addInterface(56510);
		exp.totalChildren(5);
		for (int i = 0; i < 5; i++) {
			addText(56511 + i, "Text", tda, 0, 0xffffff, false, true);
			exp.child(i, 56511, 3, 3 + (i * 13));
		}
		exp.width = 166;
		exp.height = 72;
		exp.scrollMax = 200;

		RSInterface items = addInterface(56520);
		items.totalChildren(1);
		itemGroup(56521, 4, 3, 4, 5, true, true);
		// fill(56521);
		interfaceCache[56521].contentType = 206;
		items.child(0, 56521, 5, 5);
		items.width = 166;
		items.height = 72;
		items.scrollMax = 200;

		for (int i = 0; i < 3; i++) {
			RSInterface scroll = addInterface(56023 + i);
			scroll.totalChildren(100);
			for (int j = 0; j < 100; j++) {
				addClickableText(56037 + j + (i * 100), "Achievement: " + j, "Select", tda, 0, 0xff9040, false, true,
						112);
	...
	public static void OsDropViewer(TextDrawingArea[] tda) {
		...
		addSprite(59501, 0, dir);
		addHoverButton(59502, dir, 1, 21, 21, "Close", 0, 69503, 1);
		addHoveredButton(59503, dir, 2, 21, 21, 69504);
		addText(63005, "Monster Drop Viewer", tda, 2, 0xFFA500, true, true);
		addText(63110, "Health: @whi@0", tda, 1, 0xff9040, false, true);// overrides
		addText(63111, "Combat Level: @whi@0", tda, 1, 0xff9040, false, true);
		addText(63112, "Max Hit: @whi@0", tda, 1, 0xff9040, false, true);
		addText(63113, "Aggressive: @whi@false", tda, 1, 0xff9040, false, true);

		addInputField(59806, 30, 0xFF981F, "NPC/Item Name..", 130, 28, false, false, "[A-Za-z0-9 .,]");
		//addText(42522, "Find npc/item drops", drawingArea, 0, 0xFF981F, true, false);
		int x = 7, y = 7;
		tab.totalChildren(11);
		tab.child(0, 59501, 0 + x, 0 + y);
		tab.child(1, 59502, 472 + x, 7 + y);
		tab.child(2, 59503, 472 + x, 7 + y);
		tab.child(3, 63005, 250 + x, 11 + y);
		tab.child(4, 59806, 8+x, 37+y);
		tab.child(5, 59507, 6 + x, 66 + y);
		tab.child(6, 54000, 150 + x, 86 + y);
		tab.child(7, 63110, 250 + x, 40 + y);
		tab.child(8, 63111, 250 + x, 60 + y);
		tab.child(9, 63112, 360 + x, 40 + y);
		tab.child(10, 63113, 360 + x, 60 + y);

		RSInterface results = addInterface(59507);
		results.width = 122;
		results.height = 250;
		results.scrollMax = 1800;
		results.totalChildren(200);
		for (int j = 0; j < 200; j++) {
			addClickableText(53008 + j, "", "View Drops", tda, 0, 0xff0000, false, true, 110);
			results.child(j, 53008 + j, 6, 8 + (j * 14));
		}

		RSInterface main = addInterface(54000);
		main.totalChildren(720);
		main.width = 328;
		main.height = 230;
		main.scrollMax = 2560;
		addSprite(54001, 3, dir);
		addSprite(54002, 4, dir);
		for (int i = 0; i < 40; i++) {
			main.child(i, 54001, 0, (i * 64));
			main.child(i + 40, 54002, 0, 32 + (i * 64));
		}
		addText(54003, "Amount:", tda, 0, 0xff9040, true, true);
		addText(54004, "Rarity:", tda, 0, 0xff9040, true, true);
		...
		for (int i = 0; i < 80; i++) {
			...
			addText(54400 + i, "1/200", tda, 0, 0xffffff, true, true);
			int yy = (i * 32);
			main.child(80 + i, 54010 + i, 1, 0 + yy);
			main.child(160 + i, 54100 + i, 39, 6 + yy);
			main.child(240 + i, 54003, 175, 2 + yy);
			main.child(320 + i, 54004, 234, 2 + yy);
			main.child(400 + i, 54005, 293, 2 + yy);
			main.child(480 + i, 54200 + i, 175, 14 + yy);
			main.child(560 + i, 54300 + i, 234, 14 + yy);
			main.child(640 + i, 54400 + i, 293, 14 + yy);
		}
	...
	public static void safeBox(TextDrawingArea[] wid) {
		...
		addSprite(55001, 19, "Interfaces/PreloadingGear/IMAGE");
		addHoverButton(55002, "Interfaces/Shop/IMAGE", 2, 21, 21, "Close Window", 201, 55002, 5);
		addHoverButton(55008, "Interfaces/Clan Chat/plus", 1, 21, 21, "Add more slots", 201, 55008, 5);
		addText(55003, "Safe Deposit Box", wid, 2, 0xFF981F, false, true);
		addText(55004, "These items will be saved when you leave \\n the PK-District.", wid, 0, 0xFF981F, false, true);
		addText(55005, "0/50", wid, 3, 0xFF981F, false, true);
		addText(55009, "Add more slots", wid, 0, 0xFFFFFF, false, true);
		setChildren(9, tab);
		setBounds(55001, 256 - 120, 60, 0, tab);
		setBounds(55002, 366, 66, 1, tab);
		setBounds(55003, 205, 66, 2, tab);
		setBounds(55004, 160, 82, 3, tab);
		setBounds(55005, 245, 110, 4, tab);
		setBounds(55006, 100, 128, 5, tab);
		setBounds(55008, 330, 113, 6, tab);
		setBounds(55009, 300, 100, 7, tab);
		setBounds(55010, 0, 0, 8, tab);

		RSInterface slotPurchase = addInterface(55010);
		setChildren(6, slotPurchase);
		darken(55011, 250, 210, 0x000000, (byte) 126);
		addText(55012, "Are you sure you want to purchase\\n1 extra slot for 15 blood money?", wid, 0, 0xFF981F, false,
				true);
		addHoverButton(55013, "Interfaces/Ironman/IMAGE", 4, 21, 21, "Yes", 201, 55013, 5);
		addHoverButton(55015, "Interfaces/Ironman/IMAGE", 3, 21, 21, "No", 201, 55013, 5);
		addText(55014, "YES", wid, 0, 0xFF981F, false, true);
		addText(55016, "NO", wid, 0, 0xFF981F, false, true);
		setBounds(55011, 135, 60, 0, slotPurchase);
		setBounds(55012, 170, 142, 1, slotPurchase);
		setBounds(55013, 210, 202, 2, slotPurchase); // Yes button
		setBounds(55014, 210, 190, 3, slotPurchase); // Yes text
		setBounds(55015, 278, 202, 4, slotPurchase); // No button
		setBounds(55016, 280, 190, 5, slotPurchase); // No text

		// Scroll
		RSInterface scroller = addInterface(55006);
		...
		// Item container
		addToItemGroup(55007, 5, 10, 9, 9, true, "Withdraw", null, null);
		setBounds(55007, 50, 5, 0, scroller);
	}
	...
	public static void drawNpcOnInterface(int childId, int npcId, int zoom) {
		...
		rsInterface.opacity = 0;
		rsInterface.isMouseoverTriggered = false;
		rsInterface.spriteScale = zoom;
		rsInterface.spritePitch = 150;
		rsInterface.spriteCameraRoll = 0;
