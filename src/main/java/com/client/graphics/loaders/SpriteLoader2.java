...
public class SpriteLoader2 {
	...
	public static void loadSprites() {
		try {
			Buffer index = new Buffer(FileOperations.readFile(Signlink.getCacheDirectory() + "media_archives/media_archive2.idx"));
			Buffer data = new Buffer(FileOperations.readFile(Signlink.getCacheDirectory() + "media_archives/media_archive2.dat"));
	...
	public static void createSprite(SpriteLoader2 sprite) {
		if (Configuration.DUMP_SPRITES) {
			File directory = new File(Signlink.getCacheDirectory() + "Sprites2/dump/");
			if (!directory.exists()) {
				directory.mkdir();
			}
			DataUtils.writeFile(
